...
1はプログラミング言語を作る側にとっては簡単で、単に8ビット整数列を文字列と呼ぶやり方です。C言語の `char` 配列やPHP, Luaなどの文字列が該当します。
...
1から5の分類とは別に、「文字列の中身にNUL文字を含められるか」という観点もあります。C言語の文字列はNULを終端として扱うので、中身としてNULを持てません。従って、`char *` は正確には「8ビット整数（ただし0を除く）の列」となります。C言語よりも新しい言語は文字列の途中にNULを持てることが多いですが、OSのAPIはC文字列を期待していることが多いので注意が必要です。

# 表現

文字列がメモリー上でどう表現されているかも重要であることが多いです。表すものがUnicode文字列であれば、UTF-8, UTF-16, UTF-32のいずれかで表現することになります。

最近の言語はUTF-8を採用するものが多いです。一方で、「JavaScriptにトランスパイルする」「JVMや.NETで動かす」ことを念頭に置くのであればUTF-16も選択肢に入るでしょう。UTF-32は馬鹿正直に実装するとたとえASCIIのみからなる文字列であっても1文字4バイト消費することになり、不利です。

## 不変条件で保証するか否か

表現をUTF-8やUTF-16とする場合、型の不変条件として正しさを保証するかどうかという論点があります。さっきのセクションで5に挙げたものは保証しています。1と3は保証していません。
