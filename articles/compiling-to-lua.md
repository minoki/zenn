...
LuaJITで浮動小数点数を扱う場合の注意点にも触れておきます。IEEE 754の浮動小数点数は `+0` と `-0` を区別することができ、例えば逆数等で違いが出ます。しかし、LuaJITはAArch64を含む一部のアーキテクチャーで「小さな整数値を（浮動小数点数ではなく）32ビット整数で表現する」という最適化を行なっており、しかもそれが（最適化の都合に留まらず）プログラム側に影響します。結果として、符号反転の方法で `-0` を作ることができません。
...
LuaJITはパースと同時にバイトコードを生成するのですが、内部関数があるとそれ以前に出現した `return` 文のバイトコードを修正 (fixup) する必要があり、関数の規模が大きいとそれが不可能になってエラーになる、ということのようです。
...
を仕込んでおくと良いでしょう。LuaJITのパーサーに早い段階で「この関数には内部関数がある」ということを教えてあげるわけです。この対策を施すと、エラーが発生しなくなることがわかります：
